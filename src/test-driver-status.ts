...
export class DriverStatusTest {
  ...
  async testDriverStatusFlow() {
    ...
    let availability =
      await this.driverStatusService.getDriverAvailability(testDriverId);
    let canAssign =
      await this.driverStatusService.canAssignTripToDriver(testDriverId);
    ...
    console.log(
      `   - Can Assign Trip: ${canAssign} (should be false due to inactivity)`,
    );
    ...
    console.log(`   - Is Active: ${isActive}`);
    console.log(
      `   - Availability: ${availability} (should still be AVAILABLE)`,
    );
    ...
    const availableDrivers =
      await this.driverStatusService.getAvailableDriversForTrip();
