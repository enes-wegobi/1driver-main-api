...
import { ApiProperty, ApiTags, ApiOperation, ApiQuery, ApiParam } from '@nestjs/swagger';
...
export class WebSocketController {
  ...
  @ApiParam({ name: 'userId', description: 'ID of the user to send the message to' })
  ...
  @ApiParam({ name: 'userId', description: 'ID of the user to get location for' })
  ...
  @ApiQuery({ name: 'latitude', description: 'Latitude coordinate', required: true })
  @ApiQuery({ name: 'longitude', description: 'Longitude coordinate', required: true })
  @ApiQuery({ name: 'radius', description: 'Search radius in kilometers', required: false, default: 5 })
  @ApiQuery({ name: 'userType', description: 'Type of users to find (driver or customer)', required: false, default: 'driver' })
  @ApiQuery({ name: 'onlyAvailable', description: 'Only return available drivers', required: false, default: false })
  ...
  async getNearbyAvailableDrivers(
    ...
    const drivers = await this.webSocketService.getRedisService().findNearbyAvailableDrivers(
      latitude,
      longitude,
      radius,
    );
  ...
  async getActiveDrivers() {
    const driverIds = await this.webSocketService.getRedisService().getActiveDrivers();
    
    // Get additional information for each driver
    const drivers = await Promise.all(
      driverIds.map(async (driverId) => {
        const location = await this.webSocketService.getUserLocation(driverId);
        const status = await this.webSocketService.getRedisService().getDriverAvailability(driverId);
  ...
  async updateDriverAvailability(
    ...
    try {
      await this.webSocketService.getRedisService().updateDriverAvailability(
        driverId,
        updateDto.status,
      );
  ...
  async getConnectedDrivers() {
    ...
    const clients = await Promise.all(
      sockets.map(async (socket) => {
        ...
        const status = await this.webSocketService.getRedisService().getDriverAvailability(userId);
