...
import { SMSResponseDto, SendSMSResponseDto, MessageStatusResponseDto } from './dto/sms-response.dto';
import { SMSApiResponse, SendSMSResult, MessageStatusData, SMSConfig } from './sms.interface';
...
export class SMSService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      const baseUrl = this.configService.get<string>('sms.baseUrl') || 'https://user.digitizebirdsms.com/api/v2';

      if (!apiKey || !clientId) {
        this.logger.warn('SMS API credentials not provided. SMS service will be disabled.');
  ...
  async sendSMS(dto: SendSMSDto): Promise<SMSResponseDto> {
    ...
    try {
      ...
      this.logger.info(`SMS sent successfully to ${result.MobileNumber}, MessageId: ${result.MessageId}`);
  ...
  async getMessageStatus(messageId: string): Promise<SMSResponseDto> {
    if (!this.smsEnabled) {
      this.logger.warn('SMS service is not enabled. Cannot get message status.');
      return new SMSResponseDto(
        -1,
        'SMS service is not enabled',
        null
