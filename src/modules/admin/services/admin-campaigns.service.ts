...
import { CampaignsService, CreateCampaignDto } from '../../campaigns/services/campaigns.service';
...
export class AdminCampaignsService {
  ...
  async createCampaign(createCampaignDto: CreateCampaignDto, image?: Express.Multer.File): Promise<CampaignDocument> {
    let imageUrl: string | undefined;

    if (image) {
      const fileKey = `campaigns/${await this.s3Service.uploadFileWithGeneratedKey(image)}`;
