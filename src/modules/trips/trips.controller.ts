...
export class TripsController {
  ...
  async getNearbyDrivers(
    ...
    this.logger.debug(`User ${user.userId} requested nearby drivers at [${query.latitude}, ${query.longitude}]`);
  ...
  @ApiResponse({ status: 200, description: 'Successfully subscribed to nearby driver updates' })
  async subscribeToNearbyDrivers(
    @Body() subscribeDto: SubscribeToNearbyDriversDto,
    @GetUser() user: any,
  ) {
    this.logger.debug(`User ${user.userId} subscribed to nearby driver updates at [${subscribeDto.latitude}, ${subscribeDto.longitude}]`);
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Unsubscribe from real-time updates of nearby drivers' })
  @ApiResponse({ status: 200, description: 'Successfully unsubscribed from nearby driver updates' })
  async unsubscribeFromNearbyDrivers(@GetUser() user: any) {
    this.logger.debug(`User ${user.userId} unsubscribed from nearby driver updates`);
    
    const success = await this.tripsService.unsubscribeFromNearbyDriverUpdates(user.userId);
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Create a trip room for real-time location sharing' })
  @ApiParam({ name: 'tripId', description: 'Trip ID' })
