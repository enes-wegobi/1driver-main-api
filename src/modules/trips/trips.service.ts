...
import { TripStatus, isValidStatusTransition, TripStatusMessages } from './enum/trip-status.enum';
...
export class TripsService {
  ...
  async subscribeToNearbyDriverUpdates(
    ...
    await this.redisService.getRedisClient().hSet(
    ...
    await this.redisService.getRedisClient().expire(
      `subscription:${clientId}`,
      1800,
    );
  ...
  async createTrip(createTripDto: CreateTripDto): Promise<any> {
    ...
    // Store trip in Redis
    await this.redisService.getRedisClient().set(
      `trip:${tripId}`,
      JSON.stringify(trip),
    );
    
    // Add to customer's trips list
    await this.redisService.getRedisClient().lPush(
      `customer:${createTripDto.customerId}:trips`,
      tripId,
    );
  ...
  async getTripById(tripId: string): Promise<any> {
    const tripData = await this.redisService.getRedisClient().get(`trip:${tripId}`);
  ...
  async updateTripStatus(updateDto: UpdateTripStatusDto): Promise<any> {
    ...
    if (updateDto.status === TripStatus.ACCEPTED && !updateDto.driverId) {
      throw new BadRequestException('Driver ID is required when accepting a trip');
    }
    
    if (updateDto.status === TripStatus.CANCELLED && !updateDto.cancellationReason) {
      throw new BadRequestException('Cancellation reason is required when cancelling a trip');
    }
    ...
    if (updateDto.status === TripStatus.ACCEPTED) {
      ...
      await this.redisService.getRedisClient().lPush(
        `driver:${updateDto.driverId}:trips`,
        updateDto.tripId,
      );
    ...
    if (
      ...
      await this.redisService.getRedisClient().sRem('trips:active', updateDto.tripId);
    }
    
    // Save updated trip data
    await this.redisService.getRedisClient().set(
      `trip:${updateDto.tripId}`,
      JSON.stringify(trip),
    );
   ...
   */
  async getUserActiveTrips(userId: string, userType: 'customer' | 'driver'): Promise<any[]> {
    // Get all trip IDs for the user
    const tripIds = await this.redisService.getRedisClient().lRange(
      `${userType}:${userId}:trips`,
      0,
      -1,
    );
    ...
    for (const tripId of tripIds) {
      const isActive = await this.redisService.getRedisClient().sIsMember('trips:active', tripId);
