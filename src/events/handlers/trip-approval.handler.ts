...
export class TripApprovalHandler implements OnModuleInit {
  ...
  onModuleInit() {
    this.tripEventsService.on(
      'trip.approved',
      this.handleTripApproved.bind(this),
    );
  }
  ...
  private async handleTripApproved(event: TripApprovedEvent): Promise<void> {
    ...
    try {
      this.logger.info(
        `Background processing trip approval: ${tripId} by driver ${acceptingDriverId}`,
      );
      ...
      await this.driverTripQueueService.removeAllTripsForDriver(
        acceptingDriverId,
      );
      // 2. Cleanup queues AFTER rejected lists are updated
      const affectedDrivers = await this.cleanupTripFromQueues(
        tripId,
        acceptingDriverId,
      );
      ...
      this.logger.info(
        `Trip approval background processing completed: ${tripId}`,
      );
    } catch (error) {
      this.logger.error(
        `Trip approval background processing failed for ${tripId}: ${error.message}`,
      );
    }
  }
  ...
  private async cleanupTripFromQueues(
    ...
    try {
      ...
      this.logger.info(
        `Queue cleanup completed for trip ${tripId}, affected drivers: ${affectedDrivers.length}`,
      );

      return affectedDrivers;
    } catch (error) {
      this.logger.error(
        `Queue cleanup failed for trip ${tripId}: ${error.message}`,
      );
      return [];
    }
  }
  ...
  private async updateOtherTripsRejectedLists(
    ...
    try {
      ...
      const queueStatus =
        await this.driverTripQueueService.getDriverQueueStatus(
          acceptingDriverId,
        );
      ...
      for (const queueItem of queueStatus.nextTrips) {
        if (queueItem.tripId !== acceptedTripId) {
          await this.addDriverToTripRejectedList(
            acceptingDriverId,
            queueItem.tripId,
          );
        }
      ...
      this.logger.info(
        `Added driver ${acceptingDriverId} to rejected lists of ${queueStatus.nextTrips.length - 1} trips from their own queue`,
      );
    } catch (error) {
  ...
  private async addDriverToTripRejectedList(
    ...
    try {
      ...
      if (!rejectedDriverIds.includes(driverId)) {
        ...
        this.logger.info(
          `Added driver ${driverId} to rejected list of trip ${tripId}`,
        );
      }
    } catch (error) {
      this.logger.warn(
        `Failed to add driver ${driverId} to trip ${tripId} rejected list: ${error.message}`,
      );
    }
  }
  ...
  private async updateTripRejectedDrivers(
    tripId: string,
  ...
  private async processNextTripsForDrivers(
    ...
    const driversToProcess = affectedDrivers.filter(
      (id) => id !== acceptingDriverId,
    );
    ...
    for (let i = 0; i < limit; i++) {
      ...
      try {
        ...
        this.logger.warn(
          `Next trip processing failed for driver ${driverId}: ${error.message}`,
        );
      }
    }
  }
